...
function getInstalledDependencies (nodeModulesPath) {
  ...
  getModuleDirectories(nodeModulesPath).forEach(function (directory) {
    installedDependencies[directory] = {
      ...
      // dependencies: getInstalledDependencies(
      //   path.join(nodeModulesPath, directory, 'node_modules')
      // ),
      required: false
    }
  })
...
module.exports = function checkModules (packageInfo, nodeModulesPath, parentDependencies) {
  ...
  markDependenciesPresent(requiredDependencies, installedDependencies)
