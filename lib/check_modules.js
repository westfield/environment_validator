...
function getModuleDirectories (baseDirectory, scope) {
  scope = scope || ''
  var dirs = []
  fs.readdirSync(baseDirectory).forEach(function (directory) {
    var modulePath = path.join(scope, directory)
    if (isScopedDirectory(directory)) {
      dirs = dirs.concat(
        getModuleDirectories(path.join(baseDirectory, modulePath), directory)
      )
    } else if (directory[0] !== '.') {
